...
exports.handler = async (event) => {
  ...
  try {
    const data = await callAppsScript({ action: "getCatalogs" });
    return { statusCode: 200, headers, body: JSON.stringify(data) };
  } catch (error) {
    console.error("[catalogs]", error);
    return { statusCode: 500, headers, body: JSON.stringify({ status: "ERROR", message: error.message || "No se pudieron cargar catalogos." }) };
  }
};
